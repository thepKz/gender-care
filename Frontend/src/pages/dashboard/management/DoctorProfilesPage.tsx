...
const ManagerDoctorProfilesPage: React.FC = () => {
  ...
  const [selectedDoctorFeedback, setSelectedDoctorFeedback] = useState<FeedbackData | null>(null);
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Không thể xóa bác sĩ';
  ...
  const showFeedbackDetails = async () => {
    try {
  ...
  const handleSubmit = async (values: FormValues) => {
    try {
      ...
      if (editingDoctor) {
        ...
        // Type assertion needed due to API interface mismatch
        await doctorApi.createDoctor(createData as unknown as Parameters<typeof doctorApi.createDoctor>[0]);
          ...
          <div>
            ...
            {selectedDoctorFeedback.feedbacks.length > 0 ? (
              <List
                ...
                renderItem={(feedback, index: number) => (
                  <List.Item>
                    <List.Item.Meta
