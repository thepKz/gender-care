...
export const createDoctorQA = async (qaData: any) => {
  try {
    ...
    console.log("🚀 [CREATE-QA] Starting simple QA creation...", {
      userId,
      fullName,
      age,
      gender,
    ...
    if (!consultationService) {
      throw new Error(
        "Không tìm thấy dịch vụ tư vấn online trong hệ thống. Vui lòng tạo dịch vụ trước."
      );
    }

    console.log(
      `📋 [CREATE-QA] Found service: ${consultationService.serviceName} - ${consultationService.price}đ`
    );
    ...
    console.log(
      `   🏥 Service: ${
        consultationService.serviceName
      } - ${consultationService.price.toLocaleString("vi-VN")}đ`
    );
    console.log(
      `   👤 Customer: ${fullName}, ${age} tuổi, ${
        gender === "male" ? "Nam" : "Nữ"
      }`
    );
...
export const checkSlotAvailability = async (
  ...
  try {
    console.log(
      `🔍 [CHECK-SLOT] Checking availability for ${date} ${slotTime}`
    );
...
export const getAvailableSlotsForDate = async (
  ...
  try {
    ...
    const slotsAvailability = await Promise.all(
      FIXED_TIME_SLOTS.map(async (slotTime) => {
        try {
          ...
          console.warn(
            `⚠️ [DAILY-SLOTS] Error checking slot ${slotTime}:`,
            error
          );
          return {
            slotTime,
    ...
    const availableCount = slotsAvailability.filter(
      (slot) => slot.available
    ).length;
    console.log(
      `✅ [DAILY-SLOTS] Found ${availableCount}/8 available slots for ${date}`
    );
...
export const updatePaymentStatus = async (
  ...
  try {
    ...
    if (qa.status !== "pending_payment") {
      throw new Error(
        "Yêu cầu tư vấn này đã được xử lý hoặc không thể thanh toán"
      );
    }
...
export const doctorConfirmQA = async (
  qaId: string,
  ...
  try {
    ...
    if (qa.status !== "scheduled") {
      throw new Error(
        "Yêu cầu tư vấn chưa được lên lịch hoặc không thể confirm"
      );
    }
...
export const updateQAStatus = async (
  qaId: string,
  newStatus: string,
  doctorNotes?: string
) => {
  try {
    ...
    const updatedQA = await DoctorQA.findByIdAndUpdate(qaId, updateData, {
      new: true,
    })
      .populate({
        path: "doctorId",
...
export const findLeastBookedDoctor = async (): Promise<string> => {
  console.warn(
    "⚠️ [DEPRECATED] findLeastBookedDoctor is deprecated. Use getDoctorsWorkloadStatistics instead."
  );
  try {
...
export const findBestDoctorForNextSlot = async () => {
  console.warn(
    "⚠️ [DEPRECATED] findBestDoctorForNextSlot is deprecated. Use assignDoctorToSelectedSlot instead."
  );
  throw new Error(
    "Function deprecated. Use assignDoctorToSelectedSlot instead."
  );
};
...
export const scheduleQA = async (qaId: string) => {
  throw new Error(
    "⚠️ [DEPRECATED] scheduleQA is deprecated. Slot assignment is done automatically during QA creation."
  );
};
...
export const cancelConsultationByDoctor = async (
  ...
  try {
    ...
    console.log(
      `✅ [QA-SERVICE] Cancelled consultation ${qaId} by doctor with reason: ${reason}`
    );
    return updatedQA;
  } catch (error) {
...
export const getLiveConsultations = async (doctorId?: string) => {
  try {
    console.log("🔴 [LIVE-CONSULTATIONS] Getting live consultations...", {
      doctorId,
    });
    ...
    console.log(
      `🔴 [LIVE-CONSULTATIONS] Found ${liveConsultations.length} live consultations`
    );
...
export const getTodayConsultations = async (doctorId?: string) => {
  try {
    console.log("📅 [TODAY-CONSULTATIONS] Getting today consultations...", {
      doctorId,
    });
    ...
    console.log(
      `📅 [TODAY-CONSULTATIONS] Found ${todayConsultations.length} consultations today`
    );
 ...
 */
export const checkMeetingExistence = async (
  qaId: string
): Promise<{
  hasmeeting: boolean;
  meetingData?: any;
...
export const createMeetingRecord = async (
  ...
  try {
    ...
    console.log(
      `🔐 [CREATE-MEETING] Generated password: ${meetingPassword} for QA: ${qaId}`
    );
...
export const updateMeetingParticipants = async (
  ...
  try {
    console.log(
      `🔄 [UPDATE-PARTICIPANTS] Meeting ${meetingId}: ${participantCount} participants`
    );
 ...
 */
export const completeConsultationWithMeeting = async (
  qaId: string,
  doctorNotes?: string
): Promise<{
  updatedQA: any;
  ...
  try {
    ...
    try {
      ...
      if (customerData?.email) {
        ...
        console.log(
          `✅ [SEND-THANKS] Thank you email sent to: ${customerData.email}`
        );
      } else {
        console.warn(
          "⚠️ [SEND-THANKS] No customer email found, skipping thank you email"
        );
      }
    } catch (emailError: any) {
      console.error(
        "❌ [EMAIL-ERROR] Failed to send thank you email:",
        emailError.message
      );
...
export const getDoctorsWorkloadStatistics = async (): Promise<
  Array<{
    doctorId: string;
    doctorName: string;
    bookedSlots: number;
...
> => {
  try {
    ...
    for (const doctor of allDoctors) {
      ...
      if (schedule) {
        for (const daySchedule of schedule.weekSchedule) {
          bookedSlots += daySchedule.slots.filter(
    ...
    const sortedStats = doctorsStats.sort(
      (a, b) => a.bookedSlots - b.bookedSlots
    );

    console.log(
      `✅ [WORKLOAD-STATS] Found ${sortedStats.length} doctors, sorted by workload`
    );
...
export const assignDoctorToSelectedSlot = async (
  ...
  try {
    console.log(
      `🎯 [ASSIGN-SLOT] Starting assignment for ${selectedDate} ${selectedSlot}`
    );
    ...
    console.log(
      `📋 [ASSIGN-SLOT] Found ${availableDoctors.length} available doctors for slot`
    );
    ...
    const priorityDoctors = availableDoctors
      .map((availableDoc: any) => {
        const stats = workloadStats.find(
          (stat) => stat.doctorId === availableDoc.doctorId
        );
        return {
          ...availableDoc,
    ...
    console.log(
      `🏆 [ASSIGN-SLOT] Priority order:`,
      priorityDoctors.map(
        (d: any) => `${d.doctorInfo.fullName} (${d.bookedSlots} slots)`
      )
    );
    ...
    for (const doctor of priorityDoctors) {
      try {
        const doctorSlot = doctor.availableSlots.find(
          (slot: any) => slot.slotTime === selectedSlot
        );
        if (!doctorSlot) continue;

        console.log(
          `🔄 [ASSIGN-SLOT] Trying to lock slot for ${doctor.doctorInfo.fullName}...`
        );
        ...
        if (lockSuccess) {
          ...
          console.log(
            `✅ [ASSIGN-SLOT] Successfully assigned to ${doctor.doctorInfo.fullName}`
          );
          break;
        }
      } catch (lockError) {
        console.log(
          `❌ [ASSIGN-SLOT] Failed to lock slot for ${doctor.doctorInfo.fullName}`
        );
        continue;
      }
    }
    ...
    const payment = await PaymentTracking.create({
      ...
      billNumber: `CONS-${Date.now()}-${Math.random()
        .toString(36)
        .substring(2, 8)
        .toUpperCase()}`,
    ...
    // 8. Set dynamic timeout để auto-release
    const timeoutMinutes =
      await systemConfigService.getConsultationTimeoutMinutes();
    setTimeout(async () => {
      try {
        ...
        if (qa && qa.status === "pending_payment") {
          ...
          console.log(
            `🔓 [AUTO-RELEASE] Slot ${assignedSlotId} released after ${timeoutMinutes} minutes`
          );
        }
      } catch (timeoutError) {
