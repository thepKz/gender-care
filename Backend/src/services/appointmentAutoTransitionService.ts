...
export const autoTransitionEligibleAppointments = async (): Promise<number> => {
  try {
    const now = new Date();
    console.log(
      "🤖 [AUTO-TRANSITION] Checking appointments at:",
      now.toISOString()
    );
    ...
    console.log(
      `🔍 [AUTO-TRANSITION] Found ${appointmentsToCheck.length} confirmed appointments to check`
    );
    ...
    for (const appointment of appointmentsToCheck) {
      ...
      if (now >= appointmentDateTime) {
        ...
        console.log(
          `✅ [AUTO-TRANSITION] ${
            appointment._id
          } -> consulting (${appointment.appointmentDate.toDateString()} at ${
            appointment.appointmentTime
          })`
        );
        transitionedCount++;
    ...
    if (transitionedCount > 0) {
      console.log(
        `🎯 [AUTO-TRANSITION] Successfully transitioned ${transitionedCount} appointments to consulting`
      );
    }
    ...
    console.error(
      "❌ [AUTO-TRANSITION] Error in autoTransitionEligibleAppointments:",
      error
    );
    return 0;
  }
...
export const autoCancelExpiredPayments = async (): Promise<number> => {
  try {
    const now = new Date();
    console.log(
      "💳 [AUTO-CANCEL] Checking expired payments at:",
      now.toISOString()
    );
    ...
    const expiredPayments = await PaymentTracking.find({
      ...
      $or: [{ serviceType: "appointment" }, { serviceType: "consultation" }],
    });

    console.log(
      `🔍 [AUTO-CANCEL] Found ${expiredPayments.length} expired payments to cancel`
    );
    ...
    for (const payment of expiredPayments) {
      try {
          ...
          if (appointment && appointment.status === "pending_payment") {
            ...
            if (appointment.slotId) {
              try {
                ...
                console.log(
                  `✅ [AUTO-CANCEL] Released slot ${appointment.slotId} for appointment ${appointment._id}`
                );
              } catch (slotError) {
                console.error(
                  `❌ [AUTO-CANCEL] Error releasing slot ${appointment.slotId}:`,
                  slotError
                );
              }
            } else {
              console.log(
                `⚠️ [AUTO-CANCEL] Appointment ${appointment._id} has no slotId, skipping slot release`
              );
            }

            console.log(
              `🚫 [AUTO-CANCEL] Appointment ${appointment._id} cancelled due to payment timeout`
            );
          }
        } else if (payment.serviceType === "consultation") {
          // Cancel consultation if still pending_payment
          const consultation =
            await require("../models/DoctorQA").default.findById(
              payment.recordId
            );
          if (consultation && consultation.status === "pending_payment") {
            await consultation.updateOne({ status: "cancelled" });
            console.log(
              `🚫 [AUTO-CANCEL] Consultation ${consultation._id} cancelled due to payment timeout`
            );
          }
        }
        ...
        cancelledCount++;
        console.log(
          `✅ [AUTO-CANCEL] Payment ${payment._id} (${payment.serviceType}) expired and cancelled`
        );
      } catch (error) {
        console.error(
          `❌ [AUTO-CANCEL] Error cancelling payment ${payment._id}:`,
          error
        );
      }
    }
    ...
    const expiredAppointments = await Appointments.find({
      ...
      createdAt: { $lt: timeoutAgo }, // Created more than timeout minutes ago
      $or: [
    ...
    console.log(
      `🔍 [AUTO-CANCEL] Found ${expiredAppointments.length} expired appointments without PaymentTracking to cancel`
    );
      ...
      try {
        ...
        if (appointment.slotId) {
          try {
            await releaseSlot(appointment.slotId.toString());
            console.log(
              `✅ [AUTO-CANCEL] Released slot ${appointment.slotId} for appointment ${appointment._id} (no PaymentTracking)`
            );
          } catch (slotError) {
            console.error(
              `❌ [AUTO-CANCEL] Error releasing slot ${appointment.slotId}:`,
        ...
        console.log(
          `🚫 [AUTO-CANCEL] Appointment ${appointment._id} cancelled due to timeout (no PaymentTracking)`
        );
        cancelledCount++;
      } catch (error) {
        console.error(
          `❌ [AUTO-CANCEL] Error cancelling appointment ${appointment._id}:`,
          error
        );
      }
    }
    ...
    if (cancelledCount > 0) {
      console.log(
        `🎯 [AUTO-CANCEL] Successfully cancelled ${cancelledCount} expired payments/appointments`
      );
    }
    ...
    console.error(
      "❌ [AUTO-CANCEL] Error in autoCancelExpiredPayments:",
      error
    );
    return 0;
  }
...
export const runAutomatedServices = async (): Promise<void> => {
  try {
    ...
    console.log(
      `✅ [AUTOMATED-SERVICES] Completed: ${transitionedAppointments} transitions, ${cancelledPayments} cancellations`
    );
  } catch (error) {
    console.error(
      "❌ [AUTOMATED-SERVICES] Error running automated services:",
      error
    );
  }
};
...
export const startAutoTransitionService = () => {
  ...
  console.log(
    "✅ [AUTO-SERVICES] All automated services started - running every 5 minutes"
  );
};
