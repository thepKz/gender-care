...
export class AppointmentPaymentController {
  ...
  createPaymentLink = async (req: AuthRequest, res: Response) => {
    try {
      ...
      const serviceName =
        ...
        (appointment.packageId as any)?.name ||
        "Dịch vụ y tế";
      ...
      const paymentData = await payosService.createPaymentLink({
        ...
        cancelUrl: `${
          process.env.FRONTEND_URL || "http://localhost:5173"
        }/payment/cancel?appointmentId=${appointmentId}`,
      });
      ...
      if (
        existingPayment &&
        ["pending", "failed", "cancelled"].includes(existingPayment.status)
      ) {
        console.log(
          "🔄 [AppointmentPayment] Reusing existing payment record:",
          existingPayment._id
        );
        ...
        if (
          !appointment.paymentTrackingId ||
          appointment.paymentTrackingId.toString() !==
            paymentTrackingId.toString()
        ) {
          appointment.paymentTrackingId = paymentTrackingId;
          await appointment.save();
          console.log(
            "✅ [AppointmentPayment] Updated appointment with paymentTrackingId:",
            paymentTrackingId
          );
        }
      } else {
        ...
        // Lấy timeout từ system config
        const timeoutMinutes =
          await systemConfigService.getReservationTimeoutMinutes();
        ...
        paymentTracking = await PaymentTracking.create({
          ...
          amount,
          totalAmount: amount,
          billNumber: `APT-${Date.now()}-${Math.random()
            .toString(36)
            .substring(2, 8)
            .toUpperCase()}`,
          description,
        ...
        console.log(
          "✅ [AppointmentPayment] Created new payment record:",
          paymentTracking._id
        );

        // ✅ FIX: Lưu paymentTrackingId vào appointment với proper type
        appointment.paymentTrackingId =
          paymentTracking._id as mongoose.Types.ObjectId;
        await appointment.save();
        console.log(
          "✅ [AppointmentPayment] Updated appointment with paymentTrackingId:",
          paymentTracking._id
  ...
  checkPaymentStatus = async (req: AuthRequest, res: Response) => {
    try {
      ...
      console.log(
        "🔍 [AppointmentPayment] Checking payment status for appointment:",
        appointmentId
      );
      ...
      if (
        ...
        paymentTracking.status === "pending"
      ) {
        try {
          ...
          if (paymentInfo.status === "PAID") {
            ...
            if (
              appointment.bookingType === "new_package" &&
              appointment.packageId &&
              !appointment.packagePurchaseId
            ) {
              try {
                console.log(
                  `🎯 [AppointmentPayment] Starting PackagePurchase creation for appointment ${appointment._id}`
                );
                console.log(
                  `🎯 [AppointmentPayment] Pre-creation state check:`
                );
                ...
                console.log(
                  `  - UserId: ${appointment.createdByUserId.toString()}`
                );
                console.log(`  - Amount: ${appointment.totalAmount || 0}`);

                console.log(
                  `🎯 [AppointmentPayment] Calling PackagePurchaseService.purchasePackage with:`
                );
                console.log(
                  `  - userId: ${appointment.createdByUserId.toString()}`
                );
                console.log(
                  `  - packageId: ${appointment.packageId.toString()}`
                );
                console.log(`  - amount: ${appointment.totalAmount || 0}`);

                const packagePurchase =
                  await PackagePurchaseService.purchasePackage(
                ...
                console.log(
                  `✅ [AppointmentPayment] PackagePurchase created successfully:`
                );
                console.log(`  - PackagePurchase ID: ${packagePurchase._id}`);
                console.log(
                  `  - PackagePurchase status: ${
                    (packagePurchase as any).status
                  }`
                );
                console.log(
                  `  - PackagePurchase userId: ${
                    (packagePurchase as any).userId
                  }`
                );
                console.log(
                  `  - PackagePurchase packageId: ${
                    (packagePurchase as any).packageId
                  }`
                );
                console.log(
                  `  - PackagePurchase amount: ${
                    (packagePurchase as any).amount
                  }`
                );
                ...
                console.log(
                  `✅ [AppointmentPayment] Appointment updated successfully:`
                );
                console.log(
                  `  - Old packagePurchaseId: ${oldPackagePurchaseId}`
                );
                console.log(
                  `  - New packagePurchaseId: ${packagePurchase._id}`
                );
                console.log(`  - Appointment status: ${appointment.status}`);
              } catch (packageError) {
                console.error(
                  "❌ [AppointmentPayment] Error creating PackagePurchase:",
                  packageError
                );
                console.error(
                  `❌ [AppointmentPayment] Error type: ${typeof packageError}`
                );
                console.error(
                  `❌ [AppointmentPayment] Error name: ${
                ...
                console.error(
                  `❌ [AppointmentPayment] Error message: ${
                    packageError instanceof Error
                ...
                console.error(
                  `❌ [AppointmentPayment] Error details:`,
                  packageError
                );
                console.error(
                  `❌ [AppointmentPayment] Error stack:`,
                  packageError instanceof Error
                    ? packageError.stack
                    : "No stack trace"
                );
                ...
                console.error(
                  `❌ [AppointmentPayment] Appointment state when error occurred:`
                );
              ...
              console.log(
                ...
                `(type: ${typeof appointment.bookingType})`
              );
              console.log(
                `  - packageId:`,
                appointment.packageId,
                `(type: ${typeof appointment.packageId})`
              );
              console.log(
                `  - packagePurchaseId:`,
                appointment.packagePurchaseId,
                `(type: ${typeof appointment.packagePurchaseId})`
              );
              if (appointment.bookingType !== "new_package") {
                console.log(
                  `    => bookingType khác 'new_package' (giá trị: ${appointment.bookingType})`
                );
              }
              if (!appointment.packageId) {
                console.log(
                  `    => packageId bị thiếu/null/undefined (giá trị: ${appointment.packageId})`
                );
              }
              if (appointment.packagePurchaseId) {
                console.log(
                  `    => packagePurchaseId đã tồn tại (giá trị: ${appointment.packagePurchaseId})`
                );
              }
              console.log(
                `[DEBUG][AppointmentPaymentController] Kết thúc log lý do.\n`
              );
            }
